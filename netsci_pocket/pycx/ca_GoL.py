...
def init():
    ...
    ax.cla()
    ax.pcolor(config, cmap = matplotlib.cm.Greens)
    ax.set_title('CA: Game of Life (t = 0)')
    line.set_data([], [])
...
def animate(i):
    ...
    ax.set_axis_off()
    ax.set_title('CA: Game of Life (t = ' + str(i) + ')')
    ax.pcolor(config, cmap = matplotlib.cm.Greens)
    line.set_data([], [])
...
# anim.save('ca_GoL.mp4', extra_args=['-vcodec', 'libx264'])
