...
def init():
    ...
    ax.cla()
    ax.pcolor(config, cmap = matplotlib.cm.Reds)
    ax.set_title('CA: Forest Fire (t = 0)')
    line.set_data([], [])
...
def animate(i):
    ...
    ax.set_axis_off()
    ax.set_title('CA: Forest Fire (t = ' + str(i) + ')')
    ax.pcolor(config, cmap = matplotlib.cm.Reds)
    line.set_data([], [])
...
# anim.save('ca_forestfire.mp4', extra_args=['-vcodec', 'libx264'])
