...
# Master View python script file
# ===================================================================
...
def net_struct_avr_path_length():
    ...
    javascript_src = url_for(
        'static', filename='js/large_nw_avr_path_length.js')
...
def ds_bifurcationdiagram():
    ...
    if request.method == 'POST' and form.validate():
        result = draw(form.x0.data, int(
            form.samplingStartTime.data), int(form.sampleNumber.data))
...
@app.route('/ds_cobwebplot', methods=['GET', 'POST'])
def ds_cobwebplot():
    from netsci_pocket.pycx.ds_cobwebplot import InputForm, draw

    form = InputForm(request.form)

    if request.method == 'POST' and form.validate():
        result = draw(float(form.a.data))
        return render_template('pycx_topic.html', form=form, result=result, title="DS: Cobweb Plot")
    else:
        result = None
        return render_template('pycx_topic.html', form=form, result=result, title="DS: Cobweb Plot")
...
@app.route('/ds_logistic', methods=['GET', 'POST'])
def ds_logistic():
    from netsci_pocket.pycx.ds_logistic import InputForm, draw

    form = InputForm(request.form)

    if request.method == 'POST' and form.validate():
        result = draw(float(form.x.data), float(
            form.a.data), int(form.tmax.data))
        return render_template('pycx_topic.html', form=form, result=result, title="DS: Logistic Map")
    else:
        result = None
        return render_template('pycx_topic.html', form=form, result=result, title="DS: Logistic Map")
...
@app.route('/ds_lotka_volterra', methods=['GET', 'POST'])
def ds_lotka_volterra():
    from netsci_pocket.pycx.ds_lotka_volterra import InputForm, draw

    form = InputForm(request.form)

    if request.method == 'POST' and form.validate():
        result = draw(float(form.a.data), float(form.b.data),
                      float(form.c.data), float(form.d.data))
        return render_template('pycx_topic.html', form=form, result=result, title="DS: Lotka Volterra Equation")
    else:
        result = None
        return render_template('pycx_topic.html', form=form, result=result, title="DS: Lotka Volterra Equation")
